...
class DevilsDFTD2StageInfection(Model):
    def __init__(self, obs_pop, interventions=None, devil_fitting=False):
        if interventions is None:
            dftd_fitting = not devil_fitting
            interventions = ""
        else:
            dftd_fitting = devil_fitting = False
            if isinstance(interventions, list):
                interventions = " + ".join(interventions)
            elif not isinstance(interventions, str):
                raise TypeError("interventions must be of type list or str if set.")
        ...
        self.add_parameter([
            birth_rate, death_rate_juvenile, death_rate_over_population, death_rate_susceptible, maturity_rate
        ])

        # Parameters (DFTD)
        if not devil_fitting:
            ...
            self.add_parameter([
                death_rate_diseased, death_rate_infected, incubation, infection_rate_diseased,
                infection_rate_infected, progression, DFTD_introduction
            ])
        ...
        self.add_species([Devils, Juvenile, Susceptible])
        ...
        if not devil_fitting:
            ...
            self.add_species([Diseased, Exposed, Infected])
        ...
        # Reactions (Devils)
        birth_prop = "birth_rate * (Susceptible)"
        if not devil_fitting:
            birth_prop = birth_prop.replace(")", " + Exposed + Infected)")
        if vaccination:
        ...
        self.add_reaction([
            Birth, Mature, Death_Juvenile, Death_Juvenile2, Death_Susceptible, Death_Susceptible2
        ])
        ...
        if not devil_fitting:
            ...
            self.add_reaction([
                Death_Diseased, Death_Diseased2, Death_Exposed, Death_Exposed2, Death_Infected,
                Death_Infected2, DFTD_Stage1, DFTD_Stage2, TransmissionD, TransmissionI
            ])
        ...
        # Timespan
        if dftd_fitting or devil_fitting:
            self.timespan(np.arange(0, 421, 1)) # tspan for parameter fitting
        else:
            self.timespan(np.arange(0, 1001, 1)) # tspan for predictive simulations
    ...
    def calculate_distance(self, results):
        ...
        dists = np.zeros(len(results))
        for ndx, traj in enumerate(results):
            dists[ndx] = np.linalg.norm(traj['Devils'] - obs[0][0], 2)
    ...
    def run(self, num_sims=1, **kwargs):
        ...
        return gillespy2.Results(ret_traj), attempts
